...
class Molgrafik:
    ...
    def atomruta(self,master,namn,num):
        ...
        ruta=Frame(master,bg="red",borderwidth=2,relief=GROOVE)
        ruta.pack(side=LEFT)
        atom=Frame(ruta,bg="red")
        atom.pack(side=LEFT)
        Label(atom,text=namn,font=self.stor,bg="red").pack()
        Frame(atom, height=5, bg="red").pack()
        if num>1:
            Label(ruta, text=str(num),font=self.liten,bg="red").pack(side=BOTTOM)
